...
class OrderApp(tk.Tk):
    ...
    from tkinter import Menu  # Не забудьте импортировать Menu из tkinter
    ...
    def view_order(self):
        ...
        for product, info in self.order.items():
            ...
            update_btn.pack()
            tk.Label(self.order_window, text=f" - {info['price']} руб. каждый").pack()

        tk.Label(self.order_window, text=f"Общая стоимость: {total_price} руб.").pack()
    ...
    def confirm_order(self):
        ...
        tk.Label(self.confirm_window, text=f"Общая стоимость: {total_price} руб.").pack()
    ...
    def finalize_order(self, confirm_window, total_price, pickup_point):
        if not self.order:
            messagebox.showerror("Ошибка", "Ваш заказ пуст.")
            return
        ...
        messagebox.showinfo("Заказ оформлен",
                            f"Ваш заказ №{order_number} успешно оформлен.\nПункт выдачи: {pickup_point}\nКод получения: {retrieval_code}")
    ...
    def save_order_to_pdf(self, order_number, total_price, pickup_point, delivery_time, retrieval_code, order_content):
        ...
        c.setFont("Arial", 12)
        c.drawString(100, 750, "Заказ №" + str(order_number))
        ...
        for product, info in order_content.items():
            c.drawString(120, y, f"{product}: {info['qty']} шт. - {info['price'] * info['qty']} руб.")
            y -= 15
